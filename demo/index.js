...
$PAGE->set_url('/upload_file/index.php');
$PAGE->set_context($context);
$PAGE->set_title($title);
$PAGE->set_heading($title);
$PAGE->set_pagetype('upload_file');
...
if (has_capability('moodle/upload_file:useeditor', $context)) {
    ...
    if (!empty($editabletypes)) {
        // Editor base URL.
        $editbaseurl = new moodle_url('/upload_file/edit.php', ['contextid' => $contextid]);
        $toolbar[] = [
            'name' => get_string('add'),
...
if (has_capability('moodle/upload_file:upload', $context)) {
    // Don' show upload button if there's no plugin to support any file extension.
    $accepted = $cb->get_supported_extensions_as_string($context);
    if (!empty($accepted)) {
        $importurl = new moodle_url('/upload_file/upload.php', ['contextid' => $contextid]);
        $toolbar[] = [
            'name' => get_string('upload', 'upload_file'),
            'link' => $importurl,
            'icon' => 'i/upload',
            'action' => 'upload'
        ];
...
// Render the upload_file contents.
